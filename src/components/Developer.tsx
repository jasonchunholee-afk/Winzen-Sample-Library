...
import { FgdTestMode } from './FgdTestMode';
import { DeveloperOoDiagnostics, OO_CHUNKS_CATALOG } from './DeveloperOoDiagnostics';
...
export function Developer() {
  const [activeTab, setActiveTab] = useState<'garments' | 'rules' | 'glossary' | 'fgd_test' | 'oo_chunks' | 'syntax_macros'>('fgd_test');
  ...
  const fetchAll = async () => {
    ...
    try {
      ...
      const [gData, rData, aData] = await Promise.all([
        garmentRes.json().catch(() => []),
        rulesRes.json().catch(() => []),
        abbrRes.json().catch(() => [])
      ]);
      setGarments(Array.isArray(gData) ? gData : []);
      setRules(Array.isArray(rData) ? rData : []);
      setAbbreviations(Array.isArray(aData) ? aData : []);
    } catch (err) {
      console.error("Fetch error:", err);
      setGarments(prev => Array.isArray(prev) ? prev : []);
      setRules(prev => Array.isArray(prev) ? prev : []);
      setAbbreviations(prev => Array.isArray(prev) ? prev : []);
    }
    setLoading(false);
  };
  ...
  const safeGlossary = Array.isArray(abbreviations) ? abbreviations : [];
  ...
  const filteredGlossary = safeGlossary.filter(a => {
  ...
  return (
    <div className="w-full px-6 lg:px-8 xl:px-10 py-6 space-y-6 animate-in fade-in duration-300">
      {/* Top Header Card */}
      ...
      {garments.length >= 20 && (
        <div className="bg-gradient-to-r from-indigo-50 to-purple-50 border border-indigo-100 p-6 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold text-indigo-900 flex items-center gap-2">
              <Sparkles className="w-5 h-5 text-indigo-600" />
              Taxonomy Optimization Available
            </h2>
            <p className="text-indigo-700/80 text-sm mt-1">
              The system has collected {garments.length} garments. We have enough data points to run the dynamic cluster optimizer.
            </p>
          </div>
          <button
            onClick={() => alert('Optimization trigger logic will be implemented here during the next taxonomy strategy session.')}
            className="whitespace-nowrap px-6 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-bold rounded-xl transition-colors shadow-sm"
          >
      ...
      <div className="flex border-b border-neutral-200 bg-white px-6 rounded-xl border">
        ...
        <button
          onClick={() => setActiveTab('oo_chunks')}
          className={`flex items-center gap-2 py-4 px-4 font-semibold text-sm border-b-2 -mb-px transition-colors ${
            activeTab === 'oo_chunks'
              ? 'border-neutral-900 text-neutral-900'
              : 'border-transparent text-neutral-500 hover:text-neutral-800'
        ...
        >
          ...
          OO Architecture & Diagnostics
          <span className="bg-emerald-100 text-emerald-800 text-xs px-2 py-0.5 rounded-full font-bold">
            {OO_CHUNKS_CATALOG.length} Parts
          </span>
        </button>

        <button
          onClick={() => setActiveTab('syntax_macros')}
          className={`flex items-center gap-2 py-4 px-4 font-semibold text-sm border-b-2 -mb-px transition-colors ${
            activeTab === 'syntax_macros'
              ? 'border-neutral-900 text-neutral-900'
              : 'border-transparent text-neutral-500 hover:text-neutral-800'
          }`}
        >
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-white p-4 rounded-xl border border-neutral-200">
            <div className="flex items-center gap-3 flex-1 w-full sm:w-auto">
              ...
              <div className="flex bg-neutral-100 p-1 rounded-lg text-xs font-semibold">
                ...
                >
                  Positive ({safeRules.filter(r => r.rule_type === 'positive').length})
                </button>
                <button
                ...
                >
                  Negative ({safeRules.filter(r => r.rule_type === 'negative').length})
                </button>
              </div>
            </div>
      ...
      {/* ======================= TAB 4: FGD TEST MODE ======================= */}
      {activeTab === 'fgd_test' && (
        <FgdTestMode onRuleApplied={fetchAll} />
      )}

      {/* ======================= TAB 5: OO ARCHITECTURE DIAGNOSTICS ======================= */}
      {activeTab === 'oo_chunks' && (
        <DeveloperOoDiagnostics />
      )}
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl border border-neutral-200 p-6">
            <h2 className="text-lg font-bold text-neutral-900 flex items-center gap-2 mb-4">
              ...
              Syntax & Macros Directory
            </h2>
            <p className="text-neutral-500 mb-6">
              This directory catalogs all permanent custom syntaxes and interaction macros configured for the AI Studio agent. Use these specific text triggers in your prompts to guarantee specific architectural analysis or execution flows.
            </p>
              ...
              <div className="border border-neutral-200 rounded-xl overflow-hidden">
                <div className="bg-neutral-50 border-b border-neutral-200 p-4">
                  <div className="flex items-center gap-3 flex-wrap">
                    <span className="bg-neutral-900 text-white font-mono text-sm px-3 py-1 rounded-md">
                      [Optimisation] / Optimisation
                    </span>
                    <span className="text-sm font-semibold text-neutral-700">Pre-execution Architectural Analysis</span>
                    <span className="text-xs text-neutral-500 bg-neutral-200 px-2 py-0.5 rounded-full font-medium">
                      Brackets &amp; closing *** optional
                    </span>
                  </div>
                </div>
                <div className="p-5 bg-white text-sm">
                  <p className="text-neutral-700 font-medium mb-2">Usage Patterns (Case-Insensitive):</p>
                  <pre className="bg-neutral-100 p-3 rounded-lg font-mono text-xs text-neutral-800 mb-4 overflow-x-auto">
                    [Optimisation]{'\n'}
                    ***{'\n'}
                  ...
                  <p className="text-neutral-700 font-medium mb-2">Agent Behavior Policy:</p>
                  <ul className="list-disc pl-5 space-y-2 text-neutral-600">
                    <li><strong>No Code Generation:</strong> The Agent is strictly forbidden from writing application code upon receiving this trigger.</li>
                    <li><strong>Agent Time Estimation:</strong> Evaluates file sizes, I/O latency, and tool-call sequence length to estimate generation time.</li>
                    <li><strong>App Runtime Risk Analysis:</strong> Identifies UI DOM bloat (e.g., massive dropdowns), frontend rendering bottlenecks, and DB blocking risks.</li>
                    <li><strong>Refactoring Alerts:</strong> Detects if target files are dangerously large (&gt;300 lines) and recommends splitting components first.</li>
                    <li><strong>Prompt Splitting (The Execution Plan):</strong> The Agent outputs 2 to 4 distinct, isolated prompts (e.g., Prompt 1: DB API, Prompt 2: UI) for you to copy-paste sequentially.</li>
                  </ul>
                </div>
              </div>
              ...
              <div className="border border-neutral-200 rounded-xl overflow-hidden">
                <div className="bg-neutral-50 border-b border-neutral-200 p-4">
                  <div className="flex items-center gap-3">
                    ...
                    <span className="text-sm font-semibold text-neutral-700">Post-execution Latency & Strategy Review</span>
                  </div>
                </div>
                <div className="p-5 bg-white text-sm">
                  <p className="text-neutral-700 font-medium mb-2">Usage Pattern 1: Following an [Optimisation] plan</p>
                  <p className="text-neutral-600 mb-4 bg-neutral-50 p-3 rounded border border-neutral-100">
                    "Reflect the difference between what you predicted and the actual running time. Do you recommend further optimisation?"
                  </p>
                  
                  <p className="text-neutral-700 font-medium mb-2">Usage Pattern 2: Standalone Reflection</p>
                  <p className="text-neutral-600 mb-4 bg-neutral-50 p-3 rounded border border-neutral-100">
                    "Reflect why the previous thread took so long, recommend further optimisation."
                  </p>

                  <p className="text-neutral-700 font-medium mb-2">Agent Behavior Policy:</p>
                  <ul className="list-disc pl-5 space-y-2 text-neutral-600">
                    <li><strong>Latency Tracing:</strong> The Agent analyzes why the previous operations (tool calls vs Node.js runtime) took the duration they did.</li>
                    <li><strong>Workflow Adjustments:</strong> Recommends whether future prompts of similar nature need tighter structural separation (Backend vs Frontend) or file restructuring.</li>
                    <li><strong>Conceptual Alignment:</strong> Acknowledges that 'optimisation' here refers strictly to optimizing Agent Code-Writing execution speed and Application Architectural efficiency.</li>
                  </ul>
                </div>
              </div>
